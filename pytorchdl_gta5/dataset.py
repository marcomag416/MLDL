...
class GTA5(torchDataset):
    ...
    @classmethod
    def remap_labels(cls, lbl):
        label_mapping = {label.ID: label.ID for label in cls.label_map.list_}
        label_mapping.update({i: 255 for i in range(256) if i not in label_mapping})
        remapped_lbl = np.vectorize(label_mapping.get)(lbl)
        return remapped_lbl
